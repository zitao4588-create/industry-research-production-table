...
## 2026-07-14：公开报告只展示已确认发现，失败运行不得伪装成报告

- 决策：公开报告摘要必须从 reviewed report 的“已确认发现”章节反向筛选竞品和机会；database candidate、needs review、rejected 和原始 evidence 数量不能直接成为公开报告结果。
- 决策：公开质量状态固定为 `usable`、`technical_blocked` 或 `insufficient_evidence`。没有 confirmed finding 时 `canUseReport=false`，公开证据、竞品和机会全部 fail-closed 为 0。
- 决策：技术阻塞与证据不足页面必须用通俗语言说明“本次研究没有完成”，并明确它不等于行业没有机会或停止商业化；原技术报告只作为折叠记录保留。
- 决策：来源品类相关性只由 industry/category 信号满足，market、DTC、线上电商等上下文词不能独立证明品类相关；用户明确提供的 URL 仍只能按既有 candidate 边界进入后续审核。
- 原因：旧冰箱 run 同时暴露了两个错误边界：宽泛市场词放大跨品类来源，公开摘要又绕过审核状态展示数据库候选，使技术失败看起来像一份完成的行业报告。
- 影响：提交 `b846f6c` 已推送并部署，旧冰箱 run 线上回放归零且显示技术阻塞。公开启动仍是旧 `public_web_llm` 工作流；本决策只修复公开可信度边界，不冒充 M1–M6 已完成通用生产接入。
...
- 决策：`evidence_gap`、`technical_blocked` 和 benchmark 失败不能自动映射为停止项目；只有真实用户需求、付费意愿、获客、留存与交付经济性证据才能改变商业化状态。
- 决策：五类 finding 全部进入同一审核队列；正式 `report.md` 和 `reviewed_report.md` 共用证据门禁，运行成功信息与业务结论分开呈现。
- 原因：洗碗机 G12 已存在 quote/source/claim 完整通过的产品信号，但旧审核队列遗漏 product signal，导致统一 finding 指标错误显示为 0；同时旧脚本越权生成商业冻结结论。
- 影响：报告输出更保守但不悲观；下一步基于已有洗碗机产物修复 claim 完整性和机会假设，不再新增品类 benchmark。

## 2026-07-13：G12 只判断证据流水线，不再自动判断商业去留

- 决策事实：宠物益生菌 0、洗碗机 60、日本小众护肤 15，前三项均未通过统一硬门槛；按预注册至少 3/5 PASS 的规则，0/3 后触发 benchmark 早停，后两项不运行。
- 决策事实：64 个公开请求、5 个 LLM 请求、6 次 Tavily 搜索和保守 140 Firecrawl credits 均在授权上限内；完整现金费用保守上界 ¥4.724446/¥10。
- 决策事实：超时的 2 个 Kimi 请求没有 usage，不能把可计算的 ¥0.724446 写成完整实际费用；为其保守预留 ¥4，并让对应品类成本门禁 fail-closed。
- 证据流水线结论：`evidence_pipeline_blocked`。当前自动链路尚未达到稳定付费交付标准；洗碗机虽通过来源覆盖，但完整可证实结论和可行动 finding 仍为 0。
- 商业化结论：`not_evaluated`。G12 没有测真实需求、付费意愿、获客、留存或交付毛利，无权生成停止商业化结论。
- 影响：不再新增品类 benchmark；复用洗碗机产物修复 claim 完整性、quote 绑定和机会假设生成，报告统一输出证据状态与下一验证动作。

## 2026-07-13：G12 锁定 5 品类统一 benchmark，离线准备与 live 权限分离

- 决策：G12 按固定顺序使用宠物肠胃益生菌、洗碗机、日本小众护肤品牌、男士电动剃须刀、猫咪自动饮水机 5 个品类；不得使用 `skincare-broad-negative` 或运行后换样本。
- 决策：单品类必须同时通过可信来源/深页、完整可证实结论、正文纯净度、可行动 finding、稳定性和成本/请求硬门槛；总分至少 70 不能覆盖硬门槛失败。
- 决策：统一目标为至少 3/5 PASS；一旦当前 PASS 加剩余品类已不可能达到 3，立即 kill。任何 kill 后探索必须单列 `post_kill_exploratory`，不得进入统一分数或 C5 证据。
- 决策：离线 runner 不读取 `.env`、不负责 live 采集，并拒绝 live 标志；provider、模型、API/credits、人民币预算和 dashboard 审计必须另设确认门。
- 决策：G11 已跳过且 C4 未验证，因此 benchmark 只能判断证据流水线是否 ready / needs adjustment / blocked，不能单独把产品标为 C5 或生成商业去留结论。
- 原因：历史 runner 混有 v2/v3/post-kill 分支和已推翻的宽泛护肤负例，需要隔离出可审计、不可事后改规则的 G12 权威入口。
- 影响：G12 离线准备完成并通过 257 tests；Loop 停在 live benchmark 预算门，尚未 commit、push、部署或产生任何调用费用。
