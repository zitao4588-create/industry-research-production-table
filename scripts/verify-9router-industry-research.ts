...
function createVerificationInput(goal: string): ResearchWorkflowInput {
  return {
    projectName: "宠物益生菌竞品研究 9router 验证",
    industry: "宠物健康电商",
    category: "宠物肠胃益生菌",
    market: "美国 DTC 电商",
    researchGoal: goal,
...
function printReportSummary(
  mode: "mock_9router_report" | "public_web_9router",
  result: Awaited<ReturnType<typeof run9RouterIndustryResearchWorkflow>>,
) {
  const report = result.research_reports[0];

  if (!report) {
    throw new Error("9router did not return a report.");
  }
    ...
    JSON.stringify(
      {
        ...
        usesLocalFallback:
          ...
          report.content.includes("9router 报告节点暂时失败"),
...
async function main() {
  ...
  const mode = process.argv.includes("--public-web-9router")
    ? "public_web_9router"
    : "mock_9router_report";
  const input = createVerificationInput(
    mode === "public_web_9router"
      ? "验证公开网页采集、9router 结构化抽取和 Markdown 报告降级链路"
      : "验证 9router 能基于 mock 行业数据库生成 Markdown 报告",
  );
  const result =
    mode === "public_web_9router"
      ? await runPublic9RouterIndustryResearchWorkflow(input, {
          env: process.env,
        })
      : await run9RouterIndustryResearchWorkflow(input, { env: process.env });
