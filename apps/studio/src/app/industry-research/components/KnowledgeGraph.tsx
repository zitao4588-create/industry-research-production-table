...
export function KnowledgeGraph({
  ...
  useEffect(() => {
    ...
    function resize() {
      if (disposed) return;
      const rect = stableWrap.getBoundingClientRect();
    ...
    function draw(now: number) {
      ...
      if (hover.id && drawn[hover.id] && drawn[hover.id].type !== "leaf") {
        ...
        // FIX-3: read the live count from dbRef (node.count is frozen at mount; databases fill in later)
        const liveCount = d.type === "db" ? (dbRef.current?.[Number(hover.id.slice(2))]?.count ?? d.count) : d.count;
        const label = d.label + (liveCount != null ? "  ·  " + liveCount + " 条" : "");
