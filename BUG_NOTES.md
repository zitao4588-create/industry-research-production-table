...
## 已处理：技术失败的冰箱 run 被公开页面包装成成功报告

- 现象：`industry-research-2026-07-13T16-48-11-571Z` 没有已确认发现且存在 3 个技术失败，但公开页仍显示 4 条证据和“四方小仓科技”竞品；该条目实际是 AMZ123 的跨境物流软件内容，不是冰箱竞品。
- 根因一：公开摘要直接读取原始 database 和 `run_log.counts.evidence`，没有按 reviewed report 的“已确认发现”过滤。
- 根因二：`source-quality.ts` 把 `input.market` 及其“线上电商 / DTC”等分词加入品类相关性，通用电商文本可能在没有“冰箱”信号时被误判相关。
- 处理：公开报告契约升级为 v3；公开竞品和机会必须出现在已确认章节，0 confirmed 时计数和列表全部归零；品类相关性只使用 industry/category；失败页面明确不产生行业与商业去留结论。
- 验证：冰箱/AMZ123 反向测试、公开摘要失败样本和完整回归均通过；生产 API 返回 `technical_blocked`、0 confirmed、0 competitors、0 opportunities。

## 验收备注：本地生产回放受沙箱和生产数据边界限制

- 第一次本地生产构建因沙箱无法访问 Google Fonts 失败；允许联网后同一代码构建通过，远端生产构建也通过。
- 本地 `PORT=3106` 首次受沙箱监听权限返回 `EPERM`；获准启动后，公开报告接口先因生产 Host 白名单返回 403，使用允许 Host 后因本机没有该生产 run 返回 404。
- 处理：没有降低 Host/Origin 安全默认值，也没有复制生产数据库到本机；最终直接对已部署的生产同源公开 API 做只读回放验证。
- 部署重启窗口第一次 health 返回 502，3 秒后脚本第二次请求恢复 `status=ok`，没有重复部署或改配置。
