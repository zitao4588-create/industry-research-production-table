/* micro.jsx — crisp micro-visualizations for database cards + stat strip.
   All use currentColor (= accent, hue-shifted per card). Hover "draw" via CSS. */

function _path(d, cls, extra) {
  return React.createElement("path", Object.assign({ d, className: cls, fill: "none" }, extra));
}

/* sparkline / area trend */
function Spark({ data, w = 132, h = 42, area = true }) {
...
/* vertical bars */
function VBars({ data, w = 132, h = 42 }) {
  ...
  return (
    <svg className="micro micro-vbars" viewBox={`0 0 ${w} ${h}`} preserveAspectRatio="none">
      {data.map((v, i) => {
        ...
        return <rect key={i} className="m-bar" x={i * (bw + gap)} y={h - bh} width={bw} height={bh} rx="1.5" style={{ "--i": i }} />;
      })}
    </svg>
  );
}
...
function HBars({ data, w = 132, h = 42 }) {
  const max = Math.max(...data) || 1;
  const n = data.length;
  const gap = 4;
  ...
  return (
    <svg className="micro micro-hbars" viewBox={`0 0 ${w} ${h}`} preserveAspectRatio="none">
      {data.map((v, i) => (
        <rect key={i} className="m-bar" x="0" y={i * (bh + gap)} width={Math.max(3, (v / max) * w)} height={bh} rx="1.5" style={{ "--i": i }} />
      ))}
    </svg>
  );
}
...
function StackBar({ segs, w = 132, h = 12 }) {
  ...
  return (
    <svg className="micro micro-stack" viewBox={`0 0 ${w} ${h}`} preserveAspectRatio="none">
      {segs.map((s, i) => {
        ...
        const r = <rect key={i} x={x + 0.5} y="0" width={Math.max(0, sw - 1)} height={h} rx="2.5" className="m-seg" style={{ "--o": 0.35 + (1 - i / segs.length) * 0.55, "--i": i }} />;
        x += sw;
        return r;
      })}
    </svg>
      ...
      {segs.map((s, i) => {
        ...
        const el = (
          <circle key={i} cx={size / 2} cy={size / 2} r={r} fill="none" className="m-arc"
            ...
            transform={`rotate(-90 ${size / 2} ${size / 2})`} style={{ "--o": 0.4 + (1 - i / segs.length) * 0.55, "--i": i }} />
        );
        off += len;
        return el;
      })}
...
/* mini node constellation (sources) */
function NodeMini({ w = 132, h = 44 }) {
  ...
  return (
    <svg className="micro micro-node" viewBox={`0 0 ${w} ${h}`} preserveAspectRatio="none">
      {edges.map(([a, b], i) => (
        <line key={i} x1={nodes[a][0]} y1={nodes[a][1]} x2={nodes[b][0]} y2={nodes[b][1]} className="m-edge draw" pathLength="1" style={{ "--i": i }} />
      ))}
      {nodes.map((n, i) => (
        <circle key={i} cx={n[0]} cy={n[1]} r={i === 0 ? 3.4 : 2.2} className={"m-node" + (i === 0 ? " core" : "")} style={{ "--i": i }} />
      ))}
    </svg>
  );
}

/* scatter dots (opportunities: x=competition, y=demand, r=total) */
function Scatter({ pts, w = 132, h = 44 }) {
  return (
    <svg className="micro micro-scatter" viewBox={`0 0 ${w} ${h}`} preserveAspectRatio="none">
      ...
      {pts.map((p, i) => (
        <circle key={i} cx={4 + p.x * (w - 10)} cy={h - 4 - p.y * (h - 10)} r={2 + p.r * 3.4} className="m-pt" style={{ "--i": i }} />
      ))}
    </svg>
  );
}

/* structure blocks (tree-ish grid) */
function Blocks({ w = 132, h = 44 }) {
  const cells = [
  ...
  return (
    <svg className="micro micro-blocks" viewBox={`0 0 ${w} ${h}`} preserveAspectRatio="none">
      {cells.map((c, i) => (
        <rect key={i} x={c[0]} y={c[1]} width={c[2]} height={c[3]} rx="2" className="m-block" style={{ "--i": i, "--o": i % 3 === 0 ? 0.85 : 0.4 }} />
      ))}
    </svg>
  );
}

/* ---- per-database dispatcher (reads window.IRP, derives from real data) ---- */
function DbMicro({ id }) {
  const D = window.IRP;
  switch (id) {
...
/* inline horizontal bar for table cells */
function InlineBar({ v, max = 100, w = 48, label }) {
  return (
...
/* price positioned on a min-max scale */
function PriceScale({ price, min, max }) {
...
/* tiny stat sparkline for the top strip */
function StatSpark({ data, w = 88, h = 22 }) {
  const max = Math.max(...data), min = Math.min(...data);
  const rng = max - min || 1;
...
/* radar / pentagon for opportunity dimensions */
function Radar({ axes, size = 150 }) {
  ...
  const grid = (f) => axes.map((_, i) => pt(i, R * f).map((v) => v.toFixed(1)).join(",")).join(" ");
...
Object.assign(window, { Spark, VBars, HBars, StackBar, Donut, NodeMini, Scatter, Blocks, DbMicro, StatSpark, Radar, InlineBar, PriceScale });
