/* extras.jsx — command palette (⌘K), toast system, markdown renderer */
const { useState: _uS, useEffect: _uE, useRef: _uR, useMemo: _uM } = React;

/* ---------------- Toasts ---------------- */
function showToast(msg, kind) {
  window.dispatchEvent(new CustomEvent("irp-toast", { detail: { msg, kind: kind || "ok", id: Date.now() + Math.random() } }));
}
function Toaster() {
...
function buildIndex(D) {
  ...
  const tabs = [["opportunity", "机会评分"], ["competitor", "竞品"], ["product", "产品"], ["pain", "用户痛点"], ["content", "内容信号"], ["keyword", "关键词"], ["weekly", "周报"]];
...
function CommandPalette({ open, onClose, D, onNavigate }) {
  const [q, setQ] = _uS("");
  const [sel, setSel] = _uS(0);
  const inputRef = _uR(null);
  const index = _uM(() => buildIndex(D), [D]);
  const results = _uM(() => {
  ...
  _uE(() => { if (open) { setQ(""); setSel(0); setTimeout(() => inputRef.current?.focus(), 30); } }, [open]);
  _uE(() => { setSel(0); }, [q]);
  _uE(() => {
    if (!open) return;
    const on = (e) => {
...
Object.assign(window, { showToast, Toaster, CommandPalette, renderMarkdown });
