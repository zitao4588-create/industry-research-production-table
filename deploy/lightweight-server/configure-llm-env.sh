#!/usr/bin/env bash
# 把本机 .env.local 里的 LLM 配置和免费模型路由开关写入轻量服务器生产 env。
#
# 安全边界：
# - 默认 --dry-run：只说明将执行的动作，不连接服务器、不打印任何 key。
# - --execute：先备份远端 env（root:600 权限保持），再幂等更新受管变量
...
if [[ "$MODE" == "dry-run" ]]; then
  ...
  echo "  3. 校验本次管理的 ${#VARS[@]} 个变量各自恰好存在一行"
...
echo "[3/3] 校验变量行数"
count="$(ssh "$SSH_HOST" "sudo grep -Ec '^(AGENT_FACTORY_LLM_API_KEY|AGENT_FACTORY_LLM_BASE_URL|AGENT_FACTORY_LLM_MODEL|AGENT_FACTORY_ALIYUN_FREE_MODEL_ROUTING_ENABLED|AGENT_FACTORY_ALIYUN_FREE_TIER_ONLY_CONFIRMED)=' $REMOTE_ENV")"
echo "  远端受管变量行数：${count}（期望 ${#VARS[@]}）"
